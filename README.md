...
every decision; each pipeline step is checkpointed to SQLite, so a crashed/failed run resumes without
...
| Agent | File | Role |
|---|---|---|
| Extractor | `app/extractor.py` | Document image(s) → 8 fields `{value, confidence, source_snippet}` via `gemini-2.5-flash` (vision). Anti-hallucination: literal values only; `null` when absent. |
...
| `01_hard_scan.pdf` | Skewed, low-quality scan — OCR robustness; the deterministic Validator catches OCR slips (e.g. `6109 10`) as mismatches. |
| `03_occluded_stamps.pdf` | Stamps / ink over the page (target fields remain legible). |
| `04_phone_photo.pdf` | Phone photo with glare and perspective. |
| `05_faded_thermal.pdf` | Faded thermal-style print. |
...
trade-doc-pipeline/
├─ app/
│  ├─ config.py          # env + settings (GEMINI_API_KEY, model, dpi)
...
- Model `gemini-2.5-flash` (free tier, vision) via the current **`google-genai`** SDK (not the deprecated
  `google-generativeai`). API calls retry with backoff on 5xx / per-minute rate limits and fail fast on a
  per-day quota. The free tier is rate-limited (~20 requests/day at the time of writing).
